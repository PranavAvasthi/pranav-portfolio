...
- Both canvases use `hooks/use-visible-canvas.ts`: one dirty frame per change, no recurring animation loop, IntersectionObserver suspension off-screen, and cancellation while the document is hidden. Canvas drawing is in colocated `*.utils.ts` files; `lib/animations/` stays pure. Background stars stop redrawing before night and once fully visible. Mobile uses 160 background points; desktop scales to 340–760 points. Pixel density stays capped at 1.5/2. Two detached raster layers cache ambient and distant stars at resize, with a combined approximately 11 MB buffer budget; scroll composites these layers with the existing opacity curves. Star size and brightness preserve visibility after raster scaling, with a smooth reduction toward the reading area rather than an abrupt dim central band.
...
The continuous sky is the single expressive gesture. Six sky/horizon pairs anchor it: apricot dawn `#8CBAD9`/`#F6CFAD`, clear midday `#78BCE5`/`#D8EAF0`, bronze afternoon `#8D796F`/`#B19577`, velvet dusk `#261637`/`#462A40`, blue hour `#1E2D50`/`#394562`, and deep indigo `#111D3D`/`#263557`. Text colors are selected against the composite sky gradient for at least 4.5:1 contrast; no dusk flattening layer is needed at the named stops.

Night uses fine, subtly varied stars with a quieter centre for reading. Connections and prominent glows belong only to the interactive Skills constellation; the background has no decorative line patterns or glints. The deeper blue-hour palette gives the fine stars contrast without a separate vignette. Daylight's sun shifts from ivory to amber during its existing descent, with a shaded disc and compact halo; no additional ambient loop or full-screen haze is added.
